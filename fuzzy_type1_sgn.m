...
NS(1) = -0.5;Z(1) = 0;PS(1) = 0.001;PM(1) = 0.04;PB(1) = 1;PL(1) = 2;
pace = 0.00001;
%parameter for RFNNI
ata = 0.2;
sigma(1) = 50;    	 %sigma
%weight
w1(1) = 80;w2(1) = 80;w3(1) = 80;w4(1) = 80;w5(1) = 80;w6(1) = 80;w7(1) = 80;w8(1) = 80;w9(1) = 80;
%mid point
m1(1)=50;m2(1)=150;m3(1)=250;m4(1)=50;m5(1)=150;m6(1)=250;
for j = 1:length(t)
	%eat
	if t(j) >= 800
	...
	N = [1,(-xc2/20)]; ZERO = [(xc2+1),(1-xc2)];P = [xc2/20,1];
	...
	mux8 = f3(j)*g2(j);
	mux9 = f3(j)*g3(j);
	s = mux1 + mux2 + mux3+mux4 + mux5 + mux6 + mux7 + mux8 + mux9;
    rule1(j) = w1(j)*mux1;      %rule
    rule2(j) = w2(j)*mux2;
    rule3(j) = w3(j)*mux3;
    rule4(j) = w4(j)*mux4;
    rule5(j) = w5(j)*mux5;
...
xlabel('Time(min)');
ylabel('Insulin Concentration(mg/dl)');
axis([-inf, inf, 4, 14]);
